...
public class canhao2 : MonoBehaviour
{
    ...
    private void Update()
    {
        if (dentro)
        {
            rigPlayer.transform.position = Vector3.Lerp(rigPlayer.transform.position, new Vector3(-2.706f, 4.037f, 0), 5 * Time.deltaTime);
            if (disp)
            {
                StartCoroutine(Boom());
                disp = false;
    ...
    IEnumerator Boom()
    {
        ...
        rigPlayer.AddForce(new Vector2(20, 70), ForceMode2D.Impulse);
        dentro = false;
